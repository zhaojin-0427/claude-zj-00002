...
- **五类异常可分别配置**：阈值、连续次数、单独启停；夜间异常还可自定义夜间起止小时，
  ...
  跨夜段 `23:00-06:00` 在次日凌晨判定时自动归属前一天的星期配置；留空表示全天生效）。
- **电表级策略只按表号唯一**：一栋表的归属位置以设备档案为准，同表号不能重复创建策略，
  也不会因调用方传入不同楼栋/房间而串命中（数据库另有部分唯一索引兜底，SQLite/PostgreSQL）。
- **命中追溯（快照不可变）**：每条新告警冻结实际命中的策略版本 ID、层级、作用域目标与
  **完整参数快照**（`alerts.strategy_version_id / strategy_scope / strategy_snapshot_json`，
  告警详情返回 `strategy_snapshot`）；告警去重刷新时**只更新最近检出时间，不覆写判定详情、
  版本与快照**，后续发布 v2 再次命中，v1 的未闭环告警仍保留 v1 的全部判定依据；
  告警归属（楼栋/房间）取自触发读数的位置，设备换房后历史试跑不会串楼。
- **只读试跑**：`POST /detection-strategies/trial-run` 指定电表与历史时间范围，在独立内存库中
  复用现有去噪与检测管线回放。回放严格限定在请求区间内——区间外新增/既有读数不参与去噪邻点与
  连续计数，同一区间试跑结果稳定可复现；返回预计异常、命中策略与判定依据（`evidence/detail`），
  **不写入正式告警、不改变设备状态、不改变正式读数的去噪标记**。
- **参数安全**：阈值为 NaN/Infinity 时在参数层即返回 `40000`，不落库、不污染后续策略查询。
